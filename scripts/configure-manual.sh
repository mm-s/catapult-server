...
function node_keys_ {
	...
	if [[ ! -d votingKeys ]]; then
		echo "generating voting keys for node at $port"
		mkdir -p votingKeys
		pushd ../bin > /dev/null
...
EOF
	...
	#1---------------
	certGenerate certificate $sk $sk
...
exit 1
	...
	pushd ../bin > /dev/null
		...
		r=`./catapult.tools.linker --resources $pwd/resources --type vrf --secret $hsk --linkedPublicKey $vpkpk --output $pwd/txes/vrf_tx0.bin`
		[[ $? -ne 0 ]] && echo "Error 9584 $r" && exit 1
	popd > /dev/null

	pushd ../bin > /dev/null
		echo "Generating voting tx"
		r=`./catapult.tools.linker --resources $pwd/resources --type voting --secret $hsk --linkedPublicKey $vpk --output $pwd/txes/voting_tx0.bin`
...
#	cp ./resources $nodeDir/ -R
	...
	pushd $nodeDir > /dev/null
		bootstrap_node_new__ $preset $port $roles $cus_pubkey
...
function bootstrap_node_create {
	...
	pushd _build >/dev/null
		...
		bootstrap_node_create_ $preset $port $cus_pubkey
	popd > /dev/null
}
