...
#define TEST_CLASS MemoryUtCacheTests
	...
	TEST(TEST_CLASS, WeightReturnsSizeOfTransactionsInCacheSignedByAccount) {
		// Arrange:
		MemoryUtCache cache(Default_Options);
		...
		// Act:
		auto weight = cache.modifier().weight(initializationResult.SameSigner);

		// Assert:
		EXPECT_EQ(5 * test::GetDefaultRandomTransactionSize(), weight);
	}
	...
	TEST(TEST_CLASS, AddUpdatesCounters) {
		...
		auto modifier = cache.modifier();
		EXPECT_EQ(5 * test::GetDefaultRandomTransactionSize(), modifier.weight(key));
		for (const auto& transactionInfo : transactionInfos)
			EXPECT_EQ(test::GetDefaultRandomTransactionSize(), modifier.weight(transactionInfo.pEntity->SignerPublicKey));
	}
	...
	TEST(TEST_CLASS, RemoveUpdatesCounters) {
		...
		// Sanity:
		EXPECT_EQ(13u, cache.view().size());
		EXPECT_EQ(5 * test::GetDefaultRandomTransactionSize(), cache.modifier().weight(initializationResult.SameSigner));
		...
		auto modifier = cache.modifier();
		EXPECT_EQ(3 * test::GetDefaultRandomTransactionSize(), modifier.weight(initializationResult.SameSigner));
	...
	TEST(TEST_CLASS, RemoveAllUpdatesCounters) {
		...
		// Sanity:
		EXPECT_EQ(5 * test::GetDefaultRandomTransactionSize(), modifier.weight(initializationResult.SameSigner));
		for (const auto& transactionsInfo : initializationResult.TransactionInfos)
			EXPECT_EQ(test::GetDefaultRandomTransactionSize(), modifier.weight(transactionsInfo.pEntity->SignerPublicKey));

		// Act:
		modifier.removeAll();

		// Assert:
		EXPECT_EQ(0u, modifier.weight(initializationResult.SameSigner));
		for (const auto& transactionsInfo : initializationResult.TransactionInfos)
			EXPECT_EQ(0u, modifier.weight(transactionsInfo.pEntity->SignerPublicKey));
	}
